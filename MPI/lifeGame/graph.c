	...
	tCoordinate cell;
		...
		for (int col=0; col<worldWidth; col++)
			for (int row=0; row<worldHeight; row++){
				...
				// New cell
				if (getCellAt(&cell, currentWorld, worldWidth) == CELL_EMPTY && getCellAt(&cell, newWorld, worldWidth) == CELL_LIVE)
					drawCell (&cell, CELL_NEW, renderer);
				
				// Current cell remains alive
				if (getCellAt(&cell, currentWorld, worldWidth) == CELL_LIVE && getCellAt(&cell, newWorld, worldWidth) == CELL_LIVE)				
					drawCell (&cell, CELL_LIVE, renderer);
					
				// Current cell dies
				else if (getCellAt(&cell, currentWorld, worldWidth) == CELL_LIVE && getCellAt(&cell, newWorld, worldWidth) == CELL_EMPTY)					
